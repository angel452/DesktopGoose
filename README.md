...
./Scripts/bundle.sh          # build DesktopGoose.app into build/
...
menu bar as 🪿, with **Clean the Screen**, **Shoo the Memes** and **Quit Desktop
...
Support/Info.plist    Bundle metadata. LSUIElement keeps it out of the Dock.
Scripts/bundle.sh     Assembles and ad-hoc signs the .app.
```
...
- Speech bubbles point at where the goose stood when it spoke. They do not follow
  it as it walks away.
- Ad-hoc signed. Distributing to other Macs needs a Developer ID and notarisation.
